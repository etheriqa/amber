...
public:
  ...
  template <typename RealType>
  RealType uniform(RealType max)
  {
    return uniform<RealType>(0, max);
  }

  template <typename RealType>
  RealType uniform(RealType min, RealType max)
  {
    return std::uniform_real_distribution<RealType>(min, max)(m_engine);
  }

  template <typename RealType>
  std::tuple<RealType, RealType> circle()
  {
   ...
   */
  template <typename RealType>
  std::tuple<Vector3<RealType>, RealType> hemisphere_sa(const Vector3<RealType>& u, const Vector3<RealType>& v, const Vector3<RealType>& w)
  {
  ...
  template <typename RealType>
  std::tuple<Vector3<RealType>, RealType> hemisphere_sa(const Vector3<RealType>& w)
  {
   ...
   */
  template <typename RealType>
  std::tuple<Vector3<RealType>, RealType> hemisphere_psa(const Vector3<RealType>& u, const Vector3<RealType>& v, const Vector3<RealType>& w)
  {
    const auto r0 = uniform<RealType>(), r1 = uniform<RealType>();
  ...
  template <typename RealType>
  std::tuple<Vector3<RealType>, RealType> hemisphere_psa(const Vector3<RealType>& w)
  {
    Vector3<RealType> u, v;
    std::tie(u, v) = orthonormal_basis(w);
